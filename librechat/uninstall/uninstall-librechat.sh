#!/usr/bin/env bash
# =============================================================================
# LibreChat Uninstall Script - powered by pc-fee.com
# https://pc-fee.com | https://github.com/nephilim75/scripts
#
# Macht die Installation aus install-librechat.sh vollstaendig rueckgaengig:
# Container (api, admin-panel, mongodb, meilisearch, vectordb, rag_api), die
# zugehoerigen Docker-Volumes, das Installationsverzeichnis (Repository, .env,
# librechat.yaml, docker-compose.override.yml, data-node/, meili_data_*/,
# uploads/, images/, logs/), auf Wunsch die Backups und die Docker-Images.
#
# WICHTIG - was dieses Script grundsaetzlich NICHT anfasst:
#   * jedes Verzeichnis ausserhalb der erkannten LibreChat-Installation.
#     Zusaetzlich installierte Komponenten (Code Interpreter unter
#     /opt/LibreCodeInterpreter bzw. /opt/avila-code-interpreter, das
#     Admin-Tool unter /opt/admin-lc) werden nur erkannt und gemeldet.
#   * das Docker-Netzwerk "shared_proxy" - es wird von weiteren Stacks
#     (n8n, SearXNG, Nginx Proxy Manager, ...) mitgenutzt.
#
...
# aller Chats, Nutzerkonten und hochgeladenen Dateien. Bitte vorher mit
# --dry-run pruefen und ein Backup sicherstellen.
# =============================================================================
set -euo pipefail
...
readonly ADMIN_IMAGE_MARKER="clickhouse/librechat-admin-panel"
readonly INSTALL_SCRIPT_URL="https://raw.githubusercontent.com/nephilim75/scripts/main/librechat/install/install-librechat.sh"
# Verzeichnisse zusaetzlicher Komponenten. Werden ausschliesslich gemeldet -
# dieses Script loescht nichts ausserhalb der LibreChat-Installation.
readonly EXTRA_DIRS=(
  "/opt/LibreCodeInterpreter:Code Interpreter (usnavy13)"
  "/opt/avila-code-interpreter:Code Interpreter (LibreChat-AI/avila)"
  "/opt/admin-lc:LibreChat Admin-Tool (maintenance/menu.sh)"
)
...
Entfernt eine mit install-librechat.sh installierte LibreChat-Instanz:
Container, Docker-Volumes, das Installationsverzeichnis (Repository, .env,
librechat.yaml, Override, data-node/, meili_data_*/, uploads/, images/, logs/),
auf Wunsch die Backups und die Docker-Images.

Nicht angetastet werden das Netzwerk 'shared_proxy' und jedes Verzeichnis
ausserhalb der Installation - Code Interpreter und /opt/admin-lc werden nur
gemeldet.
...
Beispiele:
  sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/nephilim75/scripts/main/librechat/uninstall/uninstall-librechat.sh)"
  sudo ./uninstall-librechat.sh --dry-run
  sudo ./uninstall-librechat.sh --dir /srv/librechat
USAGE
}
...
LOGO
printf '%b\n' "${RESET}"
printf '%b\n' "${BOLD} LibreChat Uninstall - powered by pc-fee.com${RESET}"
printf '%b\n' " ${CYAN}https://pc-fee.com${RESET} | ${CYAN}https://github.com/nephilim75/scripts${RESET}"
echo ""
echo -e " Macht die Installation aus ${BOLD}install-librechat.sh${RESET} rueckgaengig:"
echo -e " Container, ${BOLD}Docker-Volumes${RESET} und das Installationsverzeichnis mit"
echo -e " Repository, ${BOLD}.env${RESET}, librechat.yaml, Chats (${BOLD}data-node/${RESET}), Suchindex"
echo -e " und Uploads. Auf Wunsch auch Backups und Images."
echo ""
...
echo -e " Installation (Code Interpreter, /opt/admin-lc werden nur gemeldet)."
if [[ "${DRY_RUN}" -eq 1 ]]; then
  echo ""
...
# Siehe install-librechat.sh: in ein garantiert existierendes Verzeichnis
# wechseln, damit spaetere Aufrufe nicht an einer geloeschten CWD scheitern.
...
# Identische Erkennung wie im Update-Script: Container-Label -> Bind-Mount-
# Quelle -> Standardpfad. Ein Verzeichnis gilt als LibreChat-Installation, wenn
# es eine Compose-Datei mit danny-avila/librechat-Image enthaelt.
compose_file_of() {
  local d="$1" f
...
# api und admin-panel tragen unterschiedliche Images, beide zaehlen dazu.
mapfile -t LC_CONTAINERS < <(
  docker ps -a --format '{{.ID}} {{.Names}} {{.Image}}' \
    | awk -v a="${IMAGE_MARKER}" -v b="${ADMIN_IMAGE_MARKER}" 'index($3, a) || index($3, b) {print $2}'
)
...
if [[ -n "${INSTALL_DIR:-}" ]]; then
  ...
  case "${#FOUND[@]}" in
    0)
      if [[ "${#LC_CONTAINERS[@]}" -gt 0 ]]; then
        warn "Kein Installationsverzeichnis gefunden, aber es existieren noch LibreChat-Container:"
        for c in "${LC_CONTAINERS[@]}"; do echo -e "   - ${CYAN}${c}${RESET}"; done
        if ask_yesno "${BOLD}Diese verwaisten Container entfernen?${RESET}" "n"; then
          for c in "${LC_CONTAINERS[@]}"; do run docker rm -f "${c}"; done
      ...
      success "Nichts zu tun - keine LibreChat-Installation und keine Container gefunden."
      exit 0
      ;;
    1)
      INSTALL_DIR="${FOUND[0]}"
...
# dass es sich um ein echtes Unterverzeichnis handelt und nicht um /, /opt, /srv
# oder ein Home-Verzeichnis.
...
if [[ -z "${CONTAINERS}" && "${#LC_CONTAINERS[@]}" -gt 0 ]]; then
  CONTAINERS=$(printf '%s\n' "${LC_CONTAINERS[@]}")
fi
...
if [[ -n "${CONTAINERS}" ]]; then
  ...
  warn "Keine laufenden/gestoppten LibreChat-Container gefunden (evtl. schon entfernt)."
fi

# Named Volumes des Projekts. 'compose config --volumes' liefert die Kurznamen
# aus der Compose-Datei (pgdata2, librechat-data); auf dem Host tragen sie das
# Projekt-Praefix. Ueber das Label com.docker.compose.project sind sie sicher
# zuzuordnen, ohne fremde Volumes mit aehnlichem Namen zu treffen.
...
if [[ "${#PROJECT_VOLUMES[@]}" -gt 0 ]]; then
  ...
  echo -e "     ${YELLOW}Enthalten u.a. die Vektor-Datenbank (RAG) und LibreChats Laufzeitdaten.${RESET}"
fi

echo ""
info "Folgendes Verzeichnis wird entfernt: ${BOLD}${INSTALL_DIR}${RESET}"

# item <farbe> <name> <beschreibung> - eine Zeile der Verzeichnisauflistung mit
# einheitlicher Spaltenbreite, damit die Beschreibungen buendig stehen.
dirsize() { du -sh "${INSTALL_DIR}/$1" 2>/dev/null | cut -f1; }
item() { printf '     %b+- %-29s%b%b\n' "$1" "$2" "${RESET}" "$3"; }

[[ -f "${INSTALL_DIR}/.env" ]] && \
  item "${YELLOW}" ".env" "- Schluessel, Secrets, Domains"
[[ -f "${INSTALL_DIR}/librechat.yaml" ]] && \
  item "${YELLOW}" "librechat.yaml" "- Endpunkt- und Modellkonfiguration"
[[ -f "${INSTALL_DIR}/docker-compose.override.yml" ]] && \
  item "${YELLOW}" "docker-compose.override.yml" "- NPM-Netzwerk, geschlossene Host-Ports"
[[ -d "${INSTALL_DIR}/data-node" ]] && \
  item "${RED}" "data-node/" "- ${BOLD}MongoDB: alle Chats und Nutzerkonten${RESET} ($(dirsize data-node))"
while IFS= read -r d; do
  [[ -n "${d}" ]] || continue
  item "${YELLOW}" "${d}/" "- Meilisearch-Suchindex ($(dirsize "${d}"))"
done < <(find "${INSTALL_DIR}" -maxdepth 1 -type d -name 'meili_data_*' -printf '%f\n' 2>/dev/null | sort || true)
[[ -d "${INSTALL_DIR}/uploads" ]] && \
  item "${RED}" "uploads/" "- ${BOLD}hochgeladene Dateien${RESET} ($(dirsize uploads))"
[[ -d "${INSTALL_DIR}/images" ]] && \
  item "${YELLOW}" "images/" "- generierte und hochgeladene Bilder ($(dirsize images))"
[[ -d "${INSTALL_DIR}/logs" ]] && \
  item "${YELLOW}" "logs/" "- Anwendungslogs ($(dirsize logs))"
[[ -d "${INSTALL_DIR}/.git" ]] && \
  item "${YELLOW}" ".git/" "- geklontes LibreChat-Repository"
...
# Andere Container am shared_proxy-Netz: die verlieren ihren Reverse Proxy nicht,
# werden hier aber genannt, damit klar ist, was auf dem Host weiterlaeuft.
OTHERS=""
if docker network inspect "${PROXY_NETWORK}" &>/dev/null; then
  OTHERS=$(docker network inspect "${PROXY_NETWORK}" \
    --format '{{range .Containers}}{{.Name}}{{"\n"}}{{end}}' 2>/dev/null \
    | grep -v '^$' | grep -viE 'librechat|admin-panel|chat-mongodb|chat-meilisearch|vectordb|rag_api' || true)
fi
if [[ -n "${OTHERS}" ]]; then
  echo ""
...
# Zusaetzliche Komponenten: nur melden, niemals anfassen.
...
mapfile -t AVILA_CONTAINERS < <(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^avila-' || true)
mapfile -t CI_CONTAINERS < <(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -iE 'librecodeinterpreter|code-?interpreter' || true)

if [[ "${#EXTRAS_FOUND[@]}" -gt 0 || "${#AVILA_CONTAINERS[@]}" -gt 0 || "${#CI_CONTAINERS[@]}" -gt 0 ]]; then
  echo ""
  warn "Zusaetzliche LibreChat-Komponenten gefunden - ${BOLD}diese bleiben unangetastet${RESET}:"
  for e in ${EXTRAS_FOUND[@]+"${EXTRAS_FOUND[@]}"}; do
    echo -e "   - ${CYAN}${e}${RESET}"
  done
  for c in ${AVILA_CONTAINERS[@]+"${AVILA_CONTAINERS[@]}"}; do
    echo -e "   - Container ${CYAN}${c}${RESET}"
  done
  for c in ${CI_CONTAINERS[@]+"${CI_CONTAINERS[@]}"}; do
    echo -e "   - Container ${CYAN}${c}${RESET}"
  done
  warn "Sie wurden von eigenen Scripten installiert und muessen dort entfernt werden."
fi

# Images ermitteln - bewusst aus der Compose-Config, damit auch gepinnte Tags
# erfasst werden. Basis-Images wie mongo oder pgvector koennen von anderen
# Stacks mitbenutzt werden, deshalb die Trennung in zwei Gruppen.
declare -a LC_IMAGES=() SHARED_IMAGES=()
if [[ -n "${COMPOSE_FILE}" ]]; then
  while IFS= read -r img; do
    [[ -n "${img}" ]] || continue
    docker image inspect "${img}" >/dev/null 2>&1 || continue
    if [[ "${img}" == *"${IMAGE_MARKER}"* || "${img}" == *"${ADMIN_IMAGE_MARKER}"* ]]; then
      LC_IMAGES+=("${img}")
    else
      SHARED_IMAGES+=("${img}")
    fi
...
if [[ "${#LC_IMAGES[@]}" -gt 0 || "${#SHARED_IMAGES[@]}" -gt 0 ]]; then
  echo ""
  info "Lokal vorhandene Images dieses Stacks (werden nur nach expliziter Bestaetigung geloescht):"
  for img in ${LC_IMAGES[@]+"${LC_IMAGES[@]}"}; do
    echo -e "   - ${CYAN}${img}${RESET} (LibreChat-eigen)"
  done
...
echo ""
echo "------------------------------------------------------------"
echo -e "${RED}${BOLD} Achtung: Chats, Nutzerkonten und Uploads sind danach weg.${RESET}"
...
if [[ "${BACKUP_COUNT}" -gt 0 ]]; then
  if ask_yesno "Auch die ${BACKUP_COUNT} Backup(s) (${BACKUP_SIZE}) in backups/ loeschen?" "n"; then
    ...
    KEEP_DIR="$(dirname "${INSTALL_DIR}")/librechat-backups-$(date +%F_%H-%M-%S)"
    info "Backups werden vor dem Loeschen nach ${KEEP_DIR} verschoben."
  fi
fi

REMOVE_LC_IMAGES=false
if [[ "${#LC_IMAGES[@]}" -gt 0 ]]; then
  if ask_yesno "Die LibreChat-eigenen Images loeschen (${#LC_IMAGES[@]} Stueck)?" "n"; then
    REMOVE_LC_IMAGES=true
  fi
fi

REMOVE_SHARED_IMAGES=false
if [[ "${#SHARED_IMAGES[@]}" -gt 0 ]]; then
  if ask_yesno "Auch die Basis-Images (mongo, meilisearch, pgvector, ...) loeschen? Andere Stacks koennen sie nutzen." "n"; then
...
# Falls 'down -v' ein Volume nicht erwischt hat (z.B. weil es aus einem frueheren
# Projektnamen stammt), gezielt nachraeumen - ausschliesslich Volumes mit dem
# Projekt-Label dieser Installation.
...
if ${REMOVE_LC_IMAGES}; then
  info "Entferne LibreChat-eigene Images..."
  for img in "${LC_IMAGES[@]}"; do
...
if ! ${REMOVE_LC_IMAGES} && ! ${REMOVE_SHARED_IMAGES}; then
...
if [[ "${DRY_RUN}" -eq 1 ]]; then
  ...
  echo -e "${GREEN}${BOLD} LibreChat vollstaendig entfernt.${RESET}"
fi
echo "------------------------------------------------------------"
echo ""
...
echo -e " anderer Stacks (Nginx Proxy Manager, n8n, SearXNG, ...)."
echo ""
...
if [[ "${#EXTRAS_FOUND[@]}" -gt 0 || "${#AVILA_CONTAINERS[@]}" -gt 0 || "${#CI_CONTAINERS[@]}" -gt 0 ]]; then
  ...
  echo -e "   Anleitung: ${CYAN}https://github.com/nephilim75/scripts/tree/main/librechat/codeInterpreter${RESET}"
  echo ""
fi

# Uebrig gebliebene Reste, die dieses Script bewusst nicht anfasst.
if crontab -l 2>/dev/null | grep -q 'update-librechat'; then
  warn "In der root-Crontab steht noch ein Eintrag fuer update-librechat.sh."
  warn "Bitte manuell entfernen: crontab -e"
  echo ""
fi
if [[ -f /var/log/librechat-update.log ]]; then
  info "Das Update-Log /var/log/librechat-update.log bleibt erhalten."
  echo ""
fi

echo -e " Falls in Nginx Proxy Manager noch Proxy Hosts fuer die Chat- und die"
echo -e " Admin-Domain existieren, bitte dort manuell entfernen - ebenso die"
echo -e " zugehoerigen DNS-A-Records beim Domain-Provider."
echo ""
