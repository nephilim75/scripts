#!/usr/bin/env bash
# =============================================================================
# SearXNG Uninstall Script - powered by pc-fee.com
# https://pc-fee.com | https://github.com/nephilim75/scripts
#
# Entfernt eine mit install-searxng.sh installierte SearXNG-Instanz
# rueckstandsfrei: Container, das Installationsverzeichnis (config/ mit
# settings.yml und secret_key, docker-compose.yml), auf Wunsch die Backups
# und die Docker-Images.
#
# Das Docker-Netzwerk "shared_proxy" wird NICHT angefasst - es wird von
# weiteren Stacks (n8n, LibreChat, Nginx Proxy Manager, ...) mitgenutzt.
# Diese Container laufen nach dem Entfernen weiter.
#
...
readonly DEFAULT_DIR="/opt/searxng"
readonly IMAGE_REPO="searxng/searxng"
readonly INSTALL_SCRIPT_URL="https://raw.githubusercontent.com/nephilim75/scripts/main/searxng/install/install-searxng.sh"
...
Entfernt eine mit install-searxng.sh installierte SearXNG-Instanz: Container,
das Installationsverzeichnis (config/, docker-compose.yml), auf Wunsch die
Backups und die Docker-Images. Das Netzwerk 'shared_proxy' bleibt unangetastet.
...
Beispiele:
  ...
  sudo ./uninstall-searxng.sh --dir /srv/searxng
USAGE
}
...
LOGO
printf '%b\n' "${RESET}"
printf '%b\n' "${BOLD} SearXNG Uninstall - powered by pc-fee.com${RESET}"
printf '%b\n' " ${CYAN}https://pc-fee.com${RESET} | ${CYAN}https://github.com/nephilim75/scripts${RESET}"
echo ""
echo -e " Entfernt eine mit ${BOLD}install-searxng.sh${RESET} installierte SearXNG-Instanz:"
echo -e " Container, Installationsverzeichnis (${BOLD}config/${RESET} mit settings.yml und"
echo -e " secret_key), auf Wunsch Backups und Images."
echo ""
echo -e " ${YELLOW}Wird NICHT angefasst:${RESET} das Docker-Netzwerk ${BOLD}${PROXY_NETWORK}${RESET} und"
echo -e " die Container anderer Stacks (n8n, LibreChat, Nginx Proxy Manager, ...)."
if [[ "${DRY_RUN}" -eq 1 ]]; then
  echo ""
...
# Siehe install-searxng.sh: in ein garantiert existierendes Verzeichnis
...
# -> Standardpfad. Ein Verzeichnis gilt als SearXNG-Installation, wenn es eine
# Compose-Datei mit searxng/searxng-Image enthaelt.
compose_file_of() {
  local d="$1" f
  for f in docker-compose.yml docker-compose.yaml compose.yml compose.yaml; do
    if [[ -f "${d}/${f}" ]] && grep -qi "${IMAGE_REPO}" "${d}/${f}"; then
      echo "${d}/${f}"
      return 0
    fi
  done
...
detect_dirs() {
  ...
  while IFS= read -r cid; do
    ...
    [[ -n "${d}" && "${d}" == /* ]] && raw+=("${d}")
    d=$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/etc/searxng"}}{{.Source}}{{end}}{{end}}' "${cid}" 2>/dev/null || true)
    [[ -n "${d}" && "${d}" == /* ]] && raw+=("$(dirname "${d}")")
  done < <(docker ps -a --format '{{.ID}} {{.Image}}' | awk -v img="${IMAGE_REPO}" 'index($2, img) {print $1}')
...
mapfile -t SEARXNG_CONTAINERS < <(docker ps -a --format '{{.ID}} {{.Names}} {{.Image}}' | awk -v img="${IMAGE_REPO}" 'index($3, img) {print $2}')
...
if [[ -n "${INSTALL_DIR:-}" ]]; then
  ...
  case "${#FOUND[@]}" in
    0)
      if [[ "${#SEARXNG_CONTAINERS[@]}" -gt 0 ]]; then
        warn "Kein Installationsverzeichnis gefunden, aber es existieren noch SearXNG-Container:"
        for c in "${SEARXNG_CONTAINERS[@]}"; do echo -e "   - ${CYAN}${c}${RESET}"; done
        if ask_yesno "${BOLD}Diese verwaisten Container entfernen?${RESET}" "n"; then
          for c in "${SEARXNG_CONTAINERS[@]}"; do run docker rm -f "${c}"; done
      ...
      success "Nichts zu tun - keine SearXNG-Installation und keine Container gefunden."
      exit 0
      ;;
    1)
      INSTALL_DIR="${FOUND[0]}"
    ...
    *)
      echo ""
      warn "Mehrere SearXNG-Installationen gefunden:"
      for i in "${!FOUND[@]}"; do
...
if [[ -z "${CONTAINERS}" && "${#SEARXNG_CONTAINERS[@]}" -gt 0 ]]; then
  CONTAINERS=$(printf '%s\n' "${SEARXNG_CONTAINERS[@]}")
fi
...
if [[ -n "${CONTAINERS}" ]]; then
  ...
  warn "Keine laufenden/gestoppten SearXNG-Container gefunden (evtl. schon entfernt)."
fi

echo ""
info "Folgendes Verzeichnis wird entfernt: ${BOLD}${INSTALL_DIR}${RESET}"
[[ -d "${INSTALL_DIR}/config" ]] && \
  echo -e "     ${YELLOW}+- config/${RESET}      - settings.yml inkl. secret_key"
[[ -n "${COMPOSE_FILE}" ]] && \
...
if [[ -d "${INSTALL_DIR}/backups" ]]; then
  BACKUP_COUNT=$(find "${INSTALL_DIR}/backups" -maxdepth 1 -name 'searxng_*.tar.gz' -type f 2>/dev/null | wc -l | tr -d ' ')
...
mapfile -t IMAGE_LINES < <(docker images --format '{{.Repository}}:{{.Tag}}	{{.ID}}' 2>/dev/null | grep -i "${IMAGE_REPO}" || true)
if [[ "${#IMAGE_LINES[@]}" -gt 0 ]]; then
  echo ""
  info "Lokal vorhandene SearXNG-Images (werden nur nach expliziter Bestaetigung geloescht):"
...
if [[ "${BACKUP_COUNT}" -gt 0 ]]; then
  if ask_yesno "Auch die ${BACKUP_COUNT} Backup(s) (${BACKUP_SIZE}) in backups/ loeschen?" "n"; then
    ...
    KEEP_DIR="$(dirname "${INSTALL_DIR}")/searxng-backups-$(date +%F_%H-%M-%S)"
...
if [[ "${DRY_RUN}" -eq 1 ]]; then
  ...
  echo -e "${GREEN}${BOLD} SearXNG vollstaendig entfernt.${RESET}"
fi
echo "------------------------------------------------------------"
echo ""
...
echo -e " anderer Stacks (Nginx Proxy Manager, n8n, ...)."
echo ""

# Uebrig gebliebene Reste, die dieses Script bewusst nicht anfasst.
if crontab -l 2>/dev/null | grep -q 'update-searxng'; then
  warn "In der root-Crontab steht noch ein Eintrag fuer update-searxng.sh."
  warn "Bitte manuell entfernen: crontab -e"
  echo ""
fi
if [[ -f /var/log/searxng-update.log ]]; then
  info "Das Update-Log /var/log/searxng-update.log bleibt erhalten."
  echo ""
fi

echo -e " Falls in Nginx Proxy Manager noch ein Proxy Host oder eine Access List"
echo -e " fuer diese Domain existieren, bitte manuell dort entfernen."
echo ""
