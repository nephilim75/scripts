#!/usr/bin/env bash
# =============================================================================
# SearXNG Update Script - powered by pc-fee.com
# https://pc-fee.com | https://github.com/nephilim75/scripts
#
# Aktualisiert eine mit install-searxng.sh installierte SearXNG-Instanz:
#   1. Bestandsaufnahme: Installation, Container, aktuelles Image, config/
...
#   4. Erst danach: Backup von config/ (settings.yml inkl. secret_key)
#   5. Erstellt den Container neu (erzwingt auch Versionssprung ueber 'latest')
...
#   LOG_FILE       Logdatei (Default: /var/log/searxng-update.log)
#
# Der Installationspfad wird automatisch ermittelt (Container-Label ->
# Volume-Quelle -> /opt/searxng). Nur wenn mehrere Installationen gefunden
# werden, fragt das Script nach.
#
...
readonly DEFAULT_IMAGE="searxng/searxng:latest"
readonly IMAGE_REPO="searxng/searxng"

# -- Optionen ------------------------------------------------------------------
DRY_RUN=0
KEEP="${KEEP_BACKUPS:-5}"
LOG="${LOG_FILE:-/var/log/searxng-update.log}"
...
Aktualisiert eine mit install-searxng.sh installierte SearXNG-Instanz:
Backup von config/ (inkl. settings.yml/secret_key), Image-Pull, Neuerstellung
des Containers nur bei tatsaechlich neuem Image, Aufraeumen alter Images.
...
Beispiele:
  sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/nephilim75/scripts/main/searxng/update/update-searxng.sh)"
  sudo ./update-searxng.sh --dry-run
  sudo ./update-searxng.sh --dir /srv/searxng --keep 10
USAGE
}
...
LOGO
printf '%b\n' "${RESET}"
printf '%b\n' "${BOLD} SearXNG Update - powered by pc-fee.com${RESET}"
...
# Ein Verzeichnis gilt als SearXNG-Installation, wenn es eine Compose-Datei
# enthaelt, die auf ein searxng/searxng-Image verweist.
compose_file_of() {
  local d="$1" f
...
detect_dirs() {
  ...
  # 1. Container mit SearXNG-Image: Compose-Working-Dir aus dem Label lesen.
  while IFS= read -r cid; do
    [[ -n "${cid}" ]] || continue
    d=$(docker inspect --format '{{index .Config.Labels "com.docker.compose.project.working_dir"}}' "${cid}" 2>/dev/null || true)
    [[ -n "${d}" && "${d}" == /* ]] && raw+=("${d}")
    # 2. Fallback: Quelle des /etc/searxng-Mounts - dort liegt die Installation.
...
if [[ -n "${INSTALL_DIR:-}" ]]; then
  ...
  compose_file_of "${INSTALL_DIR}" >/dev/null \
    || die "In ${INSTALL_DIR} liegt keine Compose-Datei mit einem ${IMAGE_REPO}-Image."
  info "Installationspfad (vorgegeben): ${INSTALL_DIR}"
else
  mapfile -t FOUND < <(detect_dirs)
  case "${#FOUND[@]}" in
    0)
      die "Keine SearXNG-Installation gefunden.\n  Erwartet wurde eine Compose-Datei mit ${IMAGE_REPO}-Image,\n  z.B. unter ${DEFAULT_DIR}.\n  Pfad ggf. direkt angeben: --dir /pfad/zur/installation"
      ;;
    1)
      INSTALL_DIR="${FOUND[0]}"
...
# funktioniert das Script auch bei gepinnten Tags.
IMAGE=$(sed -n "s/^[[:space:]]*image:[[:space:]]*[\"']\{0,1\}\([^\"'[:space:]]*${IMAGE_REPO}[^\"'[:space:]]*\)[\"']\{0,1\}.*/\1/p" "${COMPOSE_FILE}" | head -n 1)
...
# Aktuell nur 'config' - als Schleife gehalten, damit ein spaeterer weiterer
# Sicherungspfad (z.B. bei zukuenftigen Compose-Erweiterungen) sich analog
# zu install-npm.sh/update-npm.sh ergaenzen laesst.
...
if [[ "${#BACKUP_ITEMS[@]}" -gt 0 ]]; then
  ...
  for item in "${BACKUP_ITEMS[@]}"; do
    echo -e "   - ${CYAN}${item}/${RESET} ($(du -sh "${INSTALL_DIR}/${item}" 2>/dev/null | cut -f1)) - enthaelt settings.yml inkl. secret_key"
  done
else
  warn "Kein config/ gefunden - es kann kein Backup erstellt werden."
fi

BACKUP_COUNT=0
if [[ -d "${BACKUP_DIR}" ]]; then
  BACKUP_COUNT=$(find "${BACKUP_DIR}" -maxdepth 1 -name 'searxng_*.tar.gz' -type f 2>/dev/null | wc -l | tr -d ' ')
fi
...
if [[ "${DRY_RUN}" -eq 1 ]]; then
  ...
  echo -e "   1. Backup nach ${CYAN}${BACKUP_DIR}/searxng_<zeitstempel>.tar.gz${RESET}"
...
# -- Zusammenfassung -----------------------------------------------------------
TS=$(date +%F_%H-%M-%S)
BACKUP_FILE="${BACKUP_DIR}/searxng_${TS}.tar.gz"

mapfile -t DROP_BACKUPS < <(ls -1t "${BACKUP_DIR}"/searxng_*.tar.gz 2>/dev/null | tail -n "+${KEEP}" || true)
...
if [[ "${#BACKUP_ITEMS[@]}" -gt 0 ]]; then
  ...
  echo -e "   1. ${YELLOW}kein Backup erstellt${RESET} (config/ nicht vorhanden)"
fi
echo -e "   2. der Container neu erstellt - kurze Downtime, meist wenige Sekunden"
...
echo -e "   - ${CYAN}$(basename "${COMPOSE_FILE}")${RESET}"
echo -e "   - deine settings.yml inkl. secret_key und Limiter-Einstellung"
...
mapfile -t OLD_BACKUPS < <(ls -1t "${BACKUP_DIR}"/searxng_*.tar.gz 2>/dev/null | tail -n "+$((KEEP + 1))" || true)
